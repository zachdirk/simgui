...
void file_manager::assemble()
{
    //hard code the assembly options for now, see readme for explanations
    auto assemble = subprocess::Popen(
        {"avr-gcc", "-nostartfiles", "-mmcu=atmega2560", "-x", "assembler-with-cpp", "-g", cf_.source_file_.c_str(), "-o", cf_.object_file_.c_str()}, //command and args
...
void file_manager::generate_lst()
{
    auto generate_lst = subprocess::Popen(
        {"avr-objdump", "-m", "avr6", "-S", "-l", "-d", "-g", cf_.object_file_.c_str()}, //specify command and arguments
